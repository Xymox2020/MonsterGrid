...
public class PathFinder {
    public static int[] getNextStep(int startX, int startY, int targetX, int targetY, int gridSize, List<Obstacle> obstacles, List<Monster> monsters, Player p1, Player p2) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (nx >= 0 && nx < gridSize && ny >= 0 && ny < gridSize && !visited[nx][ny]) {
                    ...
                    if (isTarget || isPassable(nx, ny, obstacles, monsters, p1, p2)) {
                        visited[nx][ny] = true;
    ...
    private static boolean isPassable(int x, int y, List<Obstacle> obstacles, List<Monster> monsters, Player p1, Player p2) {
